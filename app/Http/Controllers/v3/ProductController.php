...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (array_key_exists('fields', $req) && rtrim($req['fields']) != '') {
            ...
            if ($checkFileds) {
                Log::channel('product_history')->info('Retrive product data failed by error: ' . $checkFileds . '', ['request' => $req]);
        ...
        if (array_key_exists('search', $req) && rtrim($req['search']) != '') {
            ...
            if ($checksearch) {
                Log::channel('product_history')->info('Retrive product data failed by error: ' . $checksearch . '', ['request' => $req]);
        ...
        Log::channel('product_history')->info('Retrive product data successfully', ['request' => $req]);
    ...
    public function show($prod_code)
    {
        ...
        if (!$product) {
            Log::channel('product_history')->info('Product not found', ['product code' => $prod_code]);
            return MyHelper::response(false, 'Product not found', [], 404);
        }
        Log::channel('product_history')->info('Successfully', ['product code' => $prod_code]);
    ...
    public function store(ProductRequest $request)
    {
        ...
        if (isset($request->branch)) {
            if (!$check_branch) {
                Log::channel('product_history')->info('Branch do not exist', ['branch' => $request->branch]);
        ...
        if (isset($request->unit_id)) {
            ...
            if (!$check_unit) {
                Log::channel('product_history')->info('Product unit do not exist', ['Unit' => $request->unit_id]);
        ...
        if (isset($request->category_id)) {
            ...
            if (!$check_cate) {
                Log::channel('product_history')->info('Product category do not exist', ['id' => $request->category_id]);
        ...
        if ($check_prod) {
            Log::channel('product_history')->info('This product already exists', ['product code' => $request->product_code]);
    ...
    public function update(ProductRequest $request, $prod_code)
    {
        ...
        try {
            $product->save();
            DB::commit();
            Log::channel('product_history')->info('Update Product successfully', ['status' => 200]);
    ...
    public function destroy($prod_code)
    {
        ...
        $product = $product->showOne($prod_code);
        if (!$product) {
            Log::channel('product_history')->info('Product Not Found', ['status' => 404, 'id' => $prod_code]);
            return MyHelper::response(false, 'Product Not Found', [], 404);
        } else {
            $product->delete();
        }
        Log::channel('product_history')->info('Delete Product Successfully', ['status' => 200, 'id' => $prod_code]);
