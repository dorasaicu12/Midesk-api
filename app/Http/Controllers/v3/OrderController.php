...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (array_key_exists('fields', $req) && rtrim($req['fields']) != '') {
            ...
            if ($checkFileds) {
                Log::channel('orders_history')->info('Retrive Order Data Failed by error:' . $checkFileds . '', ['request' => $request->all()]);
        ...
        if (array_key_exists('search', $req) && rtrim($req['search']) != '') {
            ...
            if ($checksearch) {
                Log::channel('orders_history')->info('Retrive Order Data Failed by error:' . $checksearch . '', ['request' => $request->all()]);
        ...
        Log::channel('orders_history')->info('Successfully', ['request' => $request->all()]);
    ...
    public function show($ordid)
    {
        ...
        if (!$order) {
            Log::channel('orders_history')->info('Order not found', ['id' => $ordid]);
            return MyHelper::response(false, 'Order not found', [], 404);
        }
        Log::channel('orders_history')->info('Successfully', ['id' => $ordid]);
    ...
    public function store(OrderRequest $request)
    {
        ...
        if ($order) {
            Log::channel('orders_history')->info('Order already exist', ['id' => $order->id, 'order_code' => $order->ord_code]);
        ...
        if (!$id_ord_status) {
            Log::channel('orders_history')->info('Order status do not match!', ['order_status' => $ord_status]);
        ...
        if (!$id_ord_group) {
            Log::channel('orders_history')->info('Order Group do not match!', ['order_group' => $ord_group]);
        ...
        try {
            ...
            Log::channel('orders_history')->info('Created Order successfully', ['id' => $order->find($order->id)->id, 'order_code' => $ordCode]);
            ...
            DB::rollback();
            Log::channel('orders_history')->info($ex->getMessage(), []);
    ...
    public function update(Request $request, $ordid)
    {
        ...
        if (isset($request->ticket_id)) {
            ...
            if (!$check_ticket) {
                Log::channel('orders_history')->info('ticket of order not found!', []);
        ...
        // check order status
        if (!$ordid) {
            Log::channel('orders_history')->info('OrderId not found!', ['order_id' => $ord_id]);
            return MyHelper::response(false, 'OrderId not found!', [], 404);
        }
        $order = Order::where([['ord_code', $ordid], ['groupid', $groupid]])->first();
        if (!$order) {
            Log::channel('orders_history')->info('Order not found!', []);
            return MyHelper::response(false, 'Order not found!', [], 404);
        }
        if ($request->order_status) {
            ...
            $id_ord_status = OrderStatus::where([['groupid', $groupid], ['id', $ord_status]])->first();
            if (!$id_ord_status) {
                Log::channel('orders_history')->info('Order status do not match', ['ord_status' => $ord_status]);
                return MyHelper::response(false, 'Order status do not match', [], 403);
            }
        }
        // check order channel
        if ($request->order_group) {
            ...
            $id_ord_group = OrderGroup::where([['groupid', $groupid], ['id', $ord_group]])->first();
            if (!$id_ord_group) {
                Log::channel('orders_history')->info('Order Group do not match!', ['ord_group' => $ord_group]);
                return MyHelper::response(false, 'Order Group do not match!', [], 403);
            }
        }
        ...
        if (isset($ord_id)) {
            if ($order->ord_code != $ord_id) {
                ...
                if ($order2) {
                    Log::channel('orders_history')->info('Order already exist', ['order_id' => $order2->ord_code]);
        ...
        try {
            ...
            if ($product_list) {
                ...
                $idProduct = OrderDetail::where('order_id', $order->id)->first();
                if ($idProduct) {
            ...
            Log::channel('orders_history')->info('Updated Order successfully', []);
    ...
    public function destroy($orderid)
    {
        ...
        if (!$order) {
            Log::channel('orders_history')->info('Order Not Found', ['id' => $orderid]);
            return MyHelper::response(false, 'Order Not Found', [], 404);
        } else {
            OrderDetail::where('order_id', $order->id)->delete();
            $order->delete();
        }
        Log::channel('orders_history')->info('Delete Order Successfully', ['id' => $orderid]);
