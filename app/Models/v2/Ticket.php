...
class Ticket extends Model
{
    ...
    public function getDefaultFollow($req, $array)
    {
        $res = self::with($this->relation);
        /// paginate
        if (array_key_exists('page', $req) && rtrim($req['page']) != '') {
    ...
    public function getDefaultTeam($req, $array)
    {
        $res = self::with($this->relation);
        /// paginate
        if (array_key_exists('page', $req) && rtrim($req['page']) != '') {
    ...
    public function getDefaultPending($req, $array)
    {
        $res = self::with($this->relation);
        /// paginate
        if (array_key_exists('page', $req) && rtrim($req['page']) != '') {
    ...
    public function getDefaultDelete($req, $array)
    {
        $res = self::with($this->relation);
        /// paginate
        if (array_key_exists('page', $req) && rtrim($req['page']) != '') {
    ...
    public function getDefaultNoLevel($req)
    {
        $res = self::with($this->relation);
        /// paginate
        if (array_key_exists('page', $req) && rtrim($req['page']) != '') {
    ...
    public static function insert($table, $ins, $returnID = true)
    {
        if ($returnID) {
            return DB::table($table)->insertGetId($ins);
        } else {
            return DB::table($table)->insert($ins);
        }

    }
    public static function show_order_where($table, $order_by, $where, $select = "*")
    {
    ...
    public static function update_team_staff($agent_id, $team_id, $groupid)
    {
    ...
    public static function customQuery($query, $arr = '')
    {
        if (!empty($arr)) {
            $res = DB::select($query);
    ...
    public static function show_by_id($table, $where, $select = '')
    {
        $query = DB::table($table);
        if ($select) {
            $query->select(DB::raw($select));
